...
def check_consulting_email(lates_reply: str):
  ...
  all_needs_collected_result = openai.ChatCompletion.create(model="gpt-4",
                                                            messages=[{
                                                                "role":
                                                                "user",
                                                                "content":
  ...
  all_needs_collected = all_needs_collected_result["choices"][0]["message"][
      "content"]
...
def categorise_email(lates_reply: str):
  ...
  if category == "JOB_OFFER/CONSULTING":
    ...
    if all_needs_collected == "YES":
      return {
          "Step 1":
          """Forward the email to alessandrosecchi@hotmail.it, with summary of 1.What's the problem the prospect is trying to solve?  2.Their budget"""
      }
    else:
      return {
          "Step 1":
          "Generate email response to the prospect to collect further info based on guidelines",
    ...
    if category == "COLLABORATION/SPONSORSHIP":
      return {
          "Step 1":
          "Research about the prospect & company",
          "Step 2":
          "Forward the email to alessandrosecchi@hotmail.it, with the research results included"
      }
    else:
...
class GenerateEmailResponseInput(BaseModel):
  ...
  category: str = Field(
      description=
      'category of email, can ONLY be "CONSULTING FOLLOW UP" or "OTHER" ')
...
def prospect_research(email_or_name: str, company: str):
  tools = [
      Tool(
          ...
          func=search,
          description=
          "useful for when you need to answer questions about current events, data. You should ask targeted questions"
      ),
      ScrapeWebsiteTool(),
  ]
...
class EscalateInput(BaseModel):
  ...
  message: str = Field(
      description=
      "The original email thread & message that was received, cc the original copy for escalation"
  )
  original_email_address: str = Field(
      description="The email address that sent the message/email")
  additional_context: str = Field(
      description=
      "additional context about the prospect, can be the company/prospct background research OR the consulting request details like use case, budget, etc."
  )
...
class EscalateTool(BaseTool):
  ...
  def _run(self, original_email_address: str, message: str,
           additional_context: str):
...
class ReplyEmailInput(BaseModel):
  ...
  message: str = Field(
      description=
      "The generated response message to be sent to the email address")
...
class CreateEmailDraftInput(BaseModel):
  ...
  generated_reply: str = Field(description="Generated email reply to prospect")
...
class CreateEmailDraftTool(BaseTool):
  ...
  def _run(self, prospect_email_address: str, subject: str,
           generated_reply: str):
