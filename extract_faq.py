...
def extract_faq(text_data):
    ...
    You are a smart AI assistant, above is some past emails from Ale (an electrical engineer), 
    your goal is to learn & extract common FAQ about Ale 
    (include both question & answer, return results in JSON):
    """
    map_prompt_template = PromptTemplate(template=map_prompt, input_variables=["text"])

    combine_prompt = """
    The following is set of FAQ about Ale (an electrical engineer):
    {text}
...
def save_json_to_csv(data, file_name):
    with open(file_name, mode='w', newline='', encoding='windows-1252') as file:
...
ale_replies = [entry["jason_reply"] for entry in past_emails]
ale_replies_string = json.dumps(ale_replies)

faqs = extract_faq(ale_replies_string)
