...
def parse_email(email_thread):
    ...
    You are given a raw email thread that Ale reply to others, your goal is to convert it into original message / reply pairs. 
    - orignal_message: the last message sent to Ale, if it is a long email thread, only take the last message
    - ale_reply: alessandro's reply to the original message

    if there is only one message in the thread, that should be ale_reply
...
def process_csv(input_csv_path, output_csv_path):
    with open(input_csv_path, newline='', encoding='windows-1252') as csvfile:
        ...
        for row in csv_reader:
            ...
            ale_reply = json_data.get('ale_reply', '')
            # Append original row data and new columns to processed_data
            processed_data.append([original_message, ale_reply])
